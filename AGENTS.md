...
- `.codex/AGENTS.md` and `.gemini/GEMINI.md` are the global instructions for Codex and Antigravity CLI (`agy`). Keep their role and Git-workflow sections in sync with `.claude/CLAUDE.md`.
...
- `make test`: runs `tools/tests/test-*.sh` (settings merge, settings-diff, agent-dir migration). When changing `merge.jq`, update the test first.
- `make settings-diff`: shows which managed keys the next switch would reset in `~/.claude/settings.json` and `~/.gemini/antigravity-cli/settings.json`.
...
- `make hunk-skill`: re-syncs the vendored `hunk-review` skill into `agent-plugins-private` after `brew upgrade hunk` (then bump `portable-tools` and ship it).
