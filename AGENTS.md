...
- `.codex/AGENTS.md` and `.gemini/GEMINI.md` are the global instructions for Codex and Antigravity CLI (`agy`). Keep their role and Git-workflow sections in sync with `.claude/CLAUDE.md` (Claude Code's counterpart, linked to `~/.claude/CLAUDE.md`).
...
- The user-level instructions are stored at `.claude/CLAUDE.md` and linked to `~/.claude/CLAUDE.md` under the same name (`agentFiles` in `darwin/home.nix`). Because that path exists in this checkout, Claude Code reads it as **this repository's project instructions** and does not fall back to `AGENTS.md` (the fallback only applies while no `./CLAUDE.md`, `./.claude/CLAUDE.md` or `CLAUDE.local.md` exists). `AGENTS.md` therefore documents the repository for Codex, Antigravity CLI and human readers; keep it in sync with `.claude/CLAUDE.md` by hand.
- `.claude/` provides: the user-level instructions (`CLAUDE.md`), `agents/`, `hooks/`, and `statusline.sh`. Skills do not live here (see below). `.codex/` is `~/.codex` (`AGENTS.md` only; Codex custom prompts are deprecated, so Codex gets its skills from the plugin marketplaces instead). Everything else under both directories is runtime state and untracked.
