...
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。日本語で技術書の章・記事・解説文・草稿を書くとき、または推敲・リライトするときに使う。「技術記事を書いて」「原稿を推敲して」「日本語をリライトして」「LLM っぽい文章を直して」で起動。
---
...
文章が平坦で読み進める推進力に欠けるとき、読み物として読ませたいときは、姉妹スキル `cognitive-rhythm-writing`（認知リズムの設計）を併用する。本規範が土台で、緩急の設計はその上に載せる。
...
その原稿で定義した固有の術語（概念語）を議論に使うのはよい。空虚な装飾として使うのが問題である。
...
本規範は散文向けであり、スライド制作では成果物の層で適用範囲を分ける。層はファイル名ではなく成果物の性格で判定する（下のファイル名は典型例で、リポジトリごとに異なる）。

- **全面適用**：読ませる散文（記事、CFP・プロポーザル本文、企画・構成メモ、docs の解説文。例：`article.md`、`prep.md`、`outline.md`）。
- **発表台本・スピーカーノート**（例：`script.md`）：論証・冗長・空句・記号の規範を適用するが、口語の自然さと聴衆への語りかけは、そのリポジトリに話し方・語り口を定めた文書（例：`PRESENTATION_STYLE.md`、`MY_VOICE.md`、`SPEAKING.md`）があればそれを優先する。
- **スライド本文（表示テキスト）**：適用外。段落・一文一行・パラグラフライティングの前提が成り立たない。記号（ダッシュ・中黒・全角スラッシュ・英語式コロン・絵文字装飾）、LLM っぽい空句、冗長排除の観点だけを適用し、視覚設計はスライドデザインガイドに従う。
