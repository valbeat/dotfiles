...
本規範は読む文章向けであり、スライド制作では成果物の層で適用範囲を分ける。

- **自分向け記事（article.md）・CFP・prep.md・outline.md・docs**：全面適用。冒頭で緊張を1つ作る設計は、選考委員や未来の自分に読ませるフックとしてそのまま使える。
- **発表台本（script.md）・スピーカーノート**：文の拍、断定と逡巡の交互配置、未回収の緊張の管理を適用する。ただし「進行予告は駄文」の判定は適用しない。聴衆は読み返せないため、セクション移行の予告やフライトマップ（現在地の共有）は口頭発表では必要な機能である。話し方ガイド（PRESENTATION_STYLE.md、MY_VOICE.md、SPEAKING.md 等）と衝突する場合はそちらを優先する。
