...
description: 説明的な文章に緩急を設計するための規範。緩急を装飾ではなく認知モードの切替（観察→逡巡→断定→再観察）と未回収の緊張の管理として扱い、文の拍、段落の密度波形、節の入り方、緩みと駄文の判別、執筆後の機械的な点検手順を定める。読み物として読ませたい章・記事・解説文を書くとき、または「密度はあるが平坦でおもしろくない」文章を診断・修正するときに使う。土台の規範 japanese-tech-writing と併用する。「文章に緩急をつけて」「単調な文章を直して」「読ませる文章にして」で起動。
---
...
本規範は読む文章向けであり、スライド制作では成果物の層で適用範囲を分ける。層はファイル名ではなく成果物の性格で判定する（下のファイル名は典型例で、リポジトリごとに異なる）。

- **読ませる散文（記事、CFP・プロポーザル本文、企画・構成メモ、docs。例：`article.md`、`prep.md`、`outline.md`）**：全面適用。冒頭で緊張を1つ作る設計は、選考委員や未来の自分に読ませるフックとしてそのまま使える。
- **発表台本・スピーカーノート**（例：`script.md`）：文の拍、断定と逡巡の交互配置、未回収の緊張の管理を適用する。ただし「進行予告は駄文」の判定は適用しない。聴衆は読み返せないため、セクション移行の予告やフライトマップ（現在地の共有）は口頭発表では必要な機能である。そのリポジトリに話し方・語り口を定めた文書（例：`PRESENTATION_STYLE.md`、`MY_VOICE.md`、`SPEAKING.md`）があり衝突する場合はそちらを優先する。
- **スライド本文（表示テキスト）**：適用外。段落が存在しないため文レベルの規範は使えない。未回収の緊張は構成レベル（波の設計の「亀裂→積み上げ→ピーク」、発見型プレゼン）で設計する。
