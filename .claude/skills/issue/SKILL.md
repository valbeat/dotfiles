...
7. **Issueの更新**
   ...
   **既存本文を絶対に消さないこと。** 必ず以下の手順で「既存本文の保存 → 追記 → 更新」を行う。
   実行時の注意: heredoc の終端 `EOF` は必ず行頭（インデントなし）に置くこと。
   ...
   # 3. Read で /tmp/issue-body.md を確認し、既存本文が先頭に残っていることを検証してから更新
   gh issue edit <issue-number> --body-file /tmp/issue-body.md
   ```

   すでに `## 📋 詳細仕様 (AI-Enhanced)` セクションが存在する場合は、
   末尾追記ではなく該当セクションのみ書き換える（他のセクションは変更しない）。
...
- **具体性**: ファイルパス・関数名・型名は、コードベース分析（Step 3）で実在を確認したものだけを書く。
  存在しないパスやAPIを創作しない
- **プレースホルダー禁止**: `[TBD]` `[要検討]` のような穴埋めを残さない。判断できない項目はセクションごと省略し、
  「未決事項」として明示する
- **検証可能な受け入れ基準**: 各基準は「実行して確認できる文」で書く
  （悪い例: 「使いやすいこと」／良い例: 「`POST /login` に不正なパスワードで401が返る」）
- **分量の上限**: 追記セクション全体で目安200行以内。Issueのサイズに対して過剰な仕様を書かない
