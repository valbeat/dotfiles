...
全プロジェクト共通の方針だけを置く。手順の実体は skill / hook / agent 側にあり、
ここにはスキルの外で即興判断するときに効くルールだけを残す。
dotfiles リポジトリ固有の話（settings.json の管理、ブランチ切替の注意、make ターゲット）は
リポジトリの `AGENTS.md` にある。

## 開発の進め方

- 原則 TDD（t-wada 式）。テストを先に書き、失敗を確認してから実装する。詳細手順は `/dev-workflow:impl`
- 絶対に必要でない限りファイルを作らない。新規作成より既存ファイルの編集を優先する
